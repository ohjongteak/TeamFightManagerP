    ...
    public List<CharacterPersnality> SummonCharactor(int unitCount, TeamDivid teamDivid)
    {
        ...
        for (int i = 0; i < unitCount; i++)
        {
            ...
            if (teamDivid == TeamDivid.myTeam)
                characterPersnality = Instantiate(listObjTestPrefab[4], new Vector3(spawnPos.x, spawnPos.y, 0f), Quaternion.identity, objSpawnBox.transform).GetComponent<CharacterPersnality>();
            else
                characterPersnality = Instantiate(listObjTestPrefab[4], new Vector3(spawnPos.x, spawnPos.y, 0f), Quaternion.identity, objSpawnBox.transform).GetComponent<CharacterPersnality>();
